...
<body>
	...
	<div class="top-profile-img"> 
		...
		<div id="profile">
			...
			<c:if test="${ !empty myProfile }">
				<img id="profile-img" src="${ contextPath }/resources/uploadFiles/${myProfile.renameName}">
			</c:if>
	...
	<div class="mid">
		...
		<div class="minihome">
			<div class="mid-width">
				...
				<div> <!-- 칸 안늘어나게 하기위해서 크기 미리 정해놔야 할듯 -->
					<div id="my-Intro">${ loginUser.myIntro }</div>
				</div> 
			</div>
		</div>
		...
		<div class="minihome">
			<div class="minihome-btn-width">
				...
				<div class="empty"><a onclick="location.href='${contextPath}/boardWrite.hb';"><i class="bi bi-pencil-square"></i></a></div>
			</div>
		</div>
	</div>
		    	...
		    	<table class="edit">
		    		<tr>
		    			<td>
		    				<div id="edit-img" >
		    					<c:if test="${ empty myProfile }">
									<img id="previewImg" src="${ contextPath }/resources/assets/no-img.png">
								</c:if>
								<c:if test="${ !empty myProfile }">
									<img id="previewImg" src="${ contextPath }/resources/uploadFiles/${myProfile.renameName}">
								</c:if>
							</div>
							<input class="form-con" type="file"  name="file" accept="image/JPG, image/jpeg, image/png, image/gif" id="input-file">
		    				<label class="input-file-button" for="input-file"><i class="bi bi-camera-fill"></i> 이미지 선택 </label>
		    			</td>
		    		</tr>
		    		<tr>
		    			<td>
		    				<textarea name="myIntro" id="myIntro" class="edit-text" maxlength="200" style="resize: none;" <c:if test="${ empty loginUser.myIntro }">placeholder="자기소개를 입력해주세요."</c:if>><c:if test="${ !empty loginUser.myIntro }">${ loginUser.myIntro }</c:if></textarea>
		    			</td>
		    		</tr>
		    		<tr class="text-limit">
		    			<td><a class="textCount">0</a><a class="textTotal">/80자</a></td>
		    		</tr>
		    	</table>
			</div>
	      <div class="footer">
	        <button type="button" class="button" id="cancel-bnt" data-bs-dismiss="modal">취소</button>
	        <button type="button" class="button" id="submit-btn" data-bs-dismiss="modal">저장</button>
	      </div>
	      </form>
